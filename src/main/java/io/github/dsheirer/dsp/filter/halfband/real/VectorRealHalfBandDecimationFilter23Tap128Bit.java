 ...
 *
 * This filter is optimized for an 23-tap half band filter running on a CPU that supports 128-bit/4-lane floating
 * point SIMD instructions.
 */
public class VectorRealHalfBandDecimationFilter23Tap128Bit implements IRealDecimationFilter
{
    ...
    public float[] decimateReal(float[] samples)
    {
        ...
        FloatVector filter4 = FloatVector.fromArray(VECTOR_SPECIES, mCoefficients, 12);
        FloatVector filter5 = FloatVector.fromArray(VECTOR_SPECIES, mCoefficients, 16);
        FloatVector filter6 = FloatVector.fromArray(VECTOR_SPECIES, mCoefficients, 20);

        FloatVector product1, product2, product3, product4, product5, product6;
        ...
        for(int bufferPointer = 0; bufferPointer < samples.length; bufferPointer += 2)
        {
            ...
            product2 = filter2.mul(FloatVector.fromArray(VECTOR_SPECIES, mBuffer, bufferPointer + 4));
            product3 = filter3.mul(FloatVector.fromArray(VECTOR_SPECIES, mBuffer, bufferPointer + 8));
            product4 = filter4.mul(FloatVector.fromArray(VECTOR_SPECIES, mBuffer, bufferPointer + 12));
            product5 = filter5.mul(FloatVector.fromArray(VECTOR_SPECIES, mBuffer, bufferPointer + 16));
            product6 = filter6.mul(FloatVector.fromArray(VECTOR_SPECIES, mBuffer, bufferPointer + 20));

            filtered[bufferPointer / 2] = product1.add(product2).add(product3).add(product4).add(product5)
                    .add(product6).reduceLanes(VectorOperators.ADD);
        }
