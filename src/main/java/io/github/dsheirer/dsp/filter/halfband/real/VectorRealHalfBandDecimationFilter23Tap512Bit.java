 ...
 *
 * This filter is optimized for an 23-tap half band filter running on a CPU that supports 512-bit/16-lane floating
 * point SIMD instructions.
 */
public class VectorRealHalfBandDecimationFilter23Tap512Bit implements IRealDecimationFilter
{
    ...
    private float[] mCoefficients = new float[32];
     ...
     */
    public VectorRealHalfBandDecimationFilter23Tap512Bit(float[] coefficients)
    {
        ...
        int laneMultiples = VECTOR_SPECIES.length() * (coefficients.length / VECTOR_SPECIES.length() + 1);
        mBufferOverlap = Math.max(laneMultiples - 1, coefficients.length - 1);
    }
    ...
    public float[] decimateReal(float[] samples)
    {
        ...
        FloatVector filter2 = FloatVector.fromArray(VECTOR_SPECIES, mCoefficients, 16);

        FloatVector product1, product2;
        ...
        for(int bufferPointer = 0; bufferPointer < samples.length; bufferPointer += 2)
        {
            ...
            product2 = filter2.mul(FloatVector.fromArray(VECTOR_SPECIES, mBuffer, bufferPointer + 16));
            filtered[bufferPointer / 2] = product1.add(product2).reduceLanes(VectorOperators.ADD);
        }
