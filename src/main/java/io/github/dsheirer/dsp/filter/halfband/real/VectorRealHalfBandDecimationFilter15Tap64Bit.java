 ...
 *
 * This filter is optimized for an 15-tap half band filter running on a CPU that supports 64-bit/2-lane floating
 * point SIMD instructions.
 */
public class VectorRealHalfBandDecimationFilter15Tap64Bit implements IRealDecimationFilter
{
     ...
     */
    public VectorRealHalfBandDecimationFilter15Tap64Bit(float[] coefficients)
    {
    ...
    public float[] decimateReal(float[] samples)
    {
        ...
        for(int bufferPointer = 0; bufferPointer < samples.length; bufferPointer += 2)
        {
            ...
            filtered[bufferPointer / 2] = product1.add(product2).add(product3).add(product4).add(product5)
                    .add(product6).add(product7).add(product8).reduceLanes(VectorOperators.ADD);
        }
