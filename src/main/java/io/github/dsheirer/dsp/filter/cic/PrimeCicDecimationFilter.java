...
public class PrimeCicDecimationFilter
{
    ...
    public class Decimator extends Stage
    {
         ...
         * @param size of decimation.
         */
        public Decimator(int size)
        {
            mSize = size;
        }
        ...
        public float[] process(float[] samples)
        {
            ...
            float[] decimated = new float[samples.length / mSize];

            for(int x = 0; x < samples.length; x += mSize)
            {
                decimated[x / mSize] = samples[x];
            }

            int residual = samples.length % mSize;
    ...
    public class NoDecimateTwoStage extends NoDecimateStage
    {
        ...
        public float[] process(float[] samples)
        {
            ...
            for(int x = 0; x < samples.length; x++)
            {
                ...
                temp = (previousSample + samples[x]) * gain;
