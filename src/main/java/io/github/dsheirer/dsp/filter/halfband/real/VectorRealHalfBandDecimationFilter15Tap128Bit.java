 ...
 *
 * This filter is optimized for an 15-tap half band filter running on a CPU that supports 128-bit/4-lane floating
 * point SIMD instructions.
 */
public class VectorRealHalfBandDecimationFilter15Tap128Bit implements IRealDecimationFilter
{
    private static final VectorSpecies<Float> VECTOR_SPECIES = FloatVector.SPECIES_128;
    private static final int COEFFICIENTS_LENGTH = 15;
     ...
     */
    public VectorRealHalfBandDecimationFilter15Tap128Bit(float[] coefficients)
    {
    ...
    public float[] decimateReal(float[] samples)
    {
        ...
        FloatVector product1, product2, product3, product4;
        ...
        for(int bufferPointer = 0; bufferPointer < samples.length; bufferPointer += 2)
        {
            ...
            filtered[bufferPointer / 2] = product1.add(product2).add(product3).add(product4).reduceLanes(VectorOperators.ADD);
        }
