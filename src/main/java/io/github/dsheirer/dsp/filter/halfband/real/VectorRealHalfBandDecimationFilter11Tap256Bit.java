 ...
 *
 * This filter is optimized for an 11-tap half band filter running on a CPU that supports 256-bit/8-lane floating
 * point SIMD instructions.
 */
public class VectorRealHalfBandDecimationFilter11Tap256Bit implements IRealDecimationFilter
{
     ...
     */
    public VectorRealHalfBandDecimationFilter11Tap256Bit(float[] coefficients)
    {
    ...
    public float[] decimateReal(float[] samples)
    {
        ...
        for(int bufferPointer = 0; bufferPointer < samples.length; bufferPointer += 2)
        {
            ...
            product2 = filter2.mul(FloatVector.fromArray(VECTOR_SPECIES, mBuffer, bufferPointer + 3));
