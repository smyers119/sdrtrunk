...
public class SignedByteNativeBuffer implements INativeBuffer
{
     ...
     */
    public SignedByteNativeBuffer(byte[] samples, long timestamp)
    {
    ...
    private class ComplexSamplesIterator implements Iterator<ComplexSamples>
    {
        ...
        public ComplexSamples next()
        {
            ...
            for(int pointer = 0; pointer < i.length; pointer += 2)
            {
                samplesOffset = mSamplesPointer + pointer;
                i[pointer] = LOOKUP_VALUES[(0xFF & mSamples[samplesOffset])];
                q[pointer] = LOOKUP_VALUES[(0xFF & mSamples[samplesOffset + 1])];
            }

            mSamplesPointer += (FRAGMENT_SIZE * 2);

            //TODO: figure out the timestamps here ....
    ...
    private class InterleavedComplexSamplesIterator implements Iterator<InterleavedComplexSamples>
    {
        ...
        public InterleavedComplexSamples next()
        {
            ...
            {
                converted[pointer] = LOOKUP_VALUES[(0xFF & mSamples[mSamplesPointer + pointer])];
            }

            mSamplesPointer += converted.length;
